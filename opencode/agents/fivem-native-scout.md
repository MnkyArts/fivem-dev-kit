...
Load `fivem-reference` with the `skill` tool before researching natives.
...
2. For each action, find candidate natives with `fxref search "<terms>" --side client|server|shared` (fallback
   path: `${FIVEM_DEV_KIT_ROOT}/bin/fxref`). Try at least two phrasings before concluding nothing fits.
