...
Load `fivem-scripting` and `fivem-reference` with the `skill` tool before review. Load `fivem-core` when the resource uses core.
...
1. Run `fxlint <target> --json` (fallback: `${FIVEM_DEV_KIT_ROOT}/bin/fxlint`) — this is your mechanical
...
4. Walk the rulebook's §14 checklist (from the loaded `fivem-scripting` skill) and
   `reference/security-checklist.md` item by item against the code.

## Core plugins (when the resource depends on `core`)

With `dependency 'core'` / `'@core/import.lua'` in the manifest (or core itself), load and review against the
