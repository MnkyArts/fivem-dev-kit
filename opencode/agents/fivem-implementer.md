...
Load `fivem-scripting` and `fivem-reference` with the `skill` tool before implementation. Load `fivem-core` when the resource uses core.
...
3. **Copy, don't retype.** Reuse the kit's patterns with `cp` (`${FIVEM_DEV_KIT_ROOT}/skills/fivem-scripting/
...
6. **Stop cleanly when the budget runs low.** If you are near your turn limit, or a step keeps failing, stop
   after the current file and report exactly which files are finished and which remain. The main session can
   start a new small task with the remaining list — that is expected for big resources, not a failure.

## Before writing any native call

Run `fxref show <name>` (fallback: `${FIVEM_DEV_KIT_ROOT}/bin/fxref`) yourself, even for natives the plan's
...
`fivem-core` skill must be loaded with the `skill` tool, and its rules win over generic FiveM habits. **Resolve every `Core.*` call with
...
registrations *into* core (markers, text labels, blips, interactions, doors, `UI.registerPage`, `UI.onRequest`,
cron) go inside
...
callbacks tested with `Core.Utils.isCallable`.

A **page** is the resource's own frontend (core DESIGN §38): `ui/src/index.ts` default-exports
`defineUIPlugin({ pages, setup })`, the build goes to a committed `ui/dist`, and the manifest opts in with
`core_ui 'ui/dist'` + `files { 'ui/dist/**' }` — never a `ui_page` (K008), never `SetNuiFocus`/
`SendNUIMessage`/`RegisterNuiCallback`/`GetParentResourceName`/`createApp` (K012, K017), and never
`registerPage { script = …, style = … }`, which was removed (K016). Module scope is for definitions only:
every listener, timer and subscription goes in `setup(ctx)` on `ctx.scope`. Build the plugin, not core
(`npm run build -w <plugin>-ui`, then `restart <plugin>`).

`fxlint`'s **K rules must come back clean (0/0)** — no `backdrop-filter` in `ui/` (K006), no
`package.json`/`node_modules` at the resource root or under `server/` (K007), the UI wiring and the built
`ui/dist` consistent (K014, K015). Copy shapes from `resources/core_example`.

## Follow the rulebook

Follow the loaded `fivem-scripting` skill. In particular:
...
After writing (and after any fix), run `fxlint <resource_dir>` (fallback: `${FIVEM_DEV_KIT_ROOT}/bin/fxlint`).
...
- Only touch files inside the resource directory you were given. Never edit anything under
  `${FIVEM_DEV_KIT_ROOT}` (the kit itself), another resource, or `server.cfg`.
- Do not invoke the `task` tool or delegate. Do the work yourself, or report back exactly what you
