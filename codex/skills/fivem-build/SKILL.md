...
`fxnew`/`fxlint`/`fxserver` are on PATH once `bin/` is added (see the Codex adapter README); absolute fallback:
`${FIVEM_DEV_KIT_ROOT}/bin/<name>`.

Codex has no `task` tool: to delegate, explicitly ask for the named custom agent
(e.g. "Have the `fivem-native-scout` subagent verify natives for this spec: ..."). Subagents are defined in
`~/.codex/agents/` by the adapter installer with fixed models per role — never ask a subagent to switch models,
and never paste the plan or native list inline into a prompt; subagents read `PLAN.md` from disk because long
prompts plus long outputs are exactly what blows the output window. Use `/agent` to inspect running subagent
threads.

## 0. Load context + detect the framework

Invoke the `fivem-scripting` skill — it's the rulebook you and every subagent below follow. Read
...
- Ask the `fivem-native-scout` subagent to verify natives for the spec **and the detected framework**. It returns a verified
...
Subagents read this file — never paste the plan or the native list inline into a prompt.
...
Ask the `fivem-implementer` subagent to implement with: the path to `PLAN.md`, the resource dir, and **the exact
...
each other may go in parallel (ask for several subagents in one message); otherwise sequentially. The implementer
writes one file per tool call, checks syntax as it goes and self-lints with `fxlint` before reporting.

If a run reports "files remaining" (it stopped near its turn budget), start a new subagent run for only those
files: pass the plan path and the exact remaining file list again. Do not widen the slice. Never ask an
implementer for "the whole resource in one go". If a Terra implementer run stalls twice on the same files,
retry that slice once with `gpt-6-astra` at high effort (`gpt-5.6-sol` where Astra is unavailable) and note the
escalation in the handoff.

## 6. Lint + review

Run `fxlint <resource_dir>` yourself — don't just trust the implementer's self-report. Ask the
`fivem-reviewer` subagent to review the resource, giving it the lint result. On a core plugin the **K rules** (core conventions) run too
and must end at **0 errors / 0 warnings**; `fxlint` on `resources/core` itself must stay clean as well.
`fivem-implementer` and `fivem-reviewer` have `fivem-scripting`/`fivem-reference`/`fivem-core` available, so don't re-explain
core to them — point at `PLAN.md`.

## 7. Fix findings

Send the reviewer's **confirmed** findings to `fivem-implementer` in a new subagent run for the same files
...
- Subagent models are set in `~/.codex/agents/`: scout = `gpt-5.6-luna`, implementer =
  `gpt-5.6-terra`, reviewer = `gpt-5.6-terra`. Do not ask subagents to use other models, except the
  single Astra/Sol escalation in step 5. Use the `fivem-deep-reviewer` (`gpt-6-astra`, Sol fallback) only on
  explicit request;
  its quota is much smaller.
