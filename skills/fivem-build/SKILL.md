...
- Spawn `fivem-native-scout` (haiku) with the spec **and the detected framework**. It returns a verified
...
it. `--framework standalone` gets the old non-core scaffold; `[--ox-lib]` as before. Existing resource: skip
scaffolding, plan which files to touch.
...
(opus) on the resource, giving it the lint result. On a core plugin the **K rules** (core conventions) run too
and must end at **0 errors / 0 warnings**; `fxlint` on `resources/core` itself must stay clean as well.
`fivem-implementer` and `fivem-reviewer` have `fivem-core` preloaded, so don't re-explain core to them — point
at `PLAN.md`.
